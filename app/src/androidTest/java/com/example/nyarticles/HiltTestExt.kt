...
inline fun <reified T : Fragment> launchFragmentInHiltContainer(
...
): ActivityScenario<HiltArticleTestActivity> {
    val activityIntent = Intent.makeMainActivity(
        ComponentName(
            ApplicationProvider.getApplicationContext(),
            HiltArticleTestActivity::class.java
        )
    ).putExtra(
    ...
    return ActivityScenario.launch<HiltArticleTestActivity>(activityIntent).onActivity { activity ->
        fragmentFactory?.let {
